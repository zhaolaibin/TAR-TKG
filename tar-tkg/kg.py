...
class TARLinkDataset(object):
    """TAR link prediction dataset

    The dataset contains a graph depicting the connectivity of a knowledge
    base. Currently, the knowledge bases from the
    `TAR paper <https://arxiv.org/pdf/1703.06103.pdf>`_ supported are
...
def load_entity(dataset, bfs_level, relabel):
    data = TAREntityDataset(dataset)
    data.load(bfs_level, relabel)
    return data


def load_link(dataset):
    data = TARLinkDataset(dataset)
    data.load()
    return data


def load_from_local(dir, dataset):
    data = TARLinkDataset(dataset, dir)
    # if "-d" in dataset:
